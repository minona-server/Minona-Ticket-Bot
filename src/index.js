...
for (const file of events) {
	...
	log.console(log.format(`> Yüklendi &7${event.event}&f olay`));
}
...
for (const file of commands) {
	...
	log.console(log.format(`> Yüklendi &7${config.prefix}${command.name}&f komutu`));
}

log.info(`${events.length} olayları ve ${commands.length} komutları yüklendi`);
...
const clean = () => {
	...
	if (total > 0) log.info(`${total} eski metin ${utils.plural('transcript', total)} silindi`);
};
