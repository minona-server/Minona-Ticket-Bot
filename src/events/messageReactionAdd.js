...
module.exports = {
	...
	async execute(client, log, [r, u], {config, Ticket, Setting}) {
			...
			try {
				return dm.send(
					new MessageEmbed()
						...
						.setTitle(`❌ **Zaten ${tickets.count} veya daha fazla açık biletiniz var**`)
						.setDescription(`Gereksiz biletleri kapatmak için bir sunucu kanalında \`${config.prefix}close\` kullanın.\n\n${ticketList.join(',\n')}`)
						.setFooter(channel.guild.name, channel.guild.iconURL())
				);
			} catch (e) {
				let m = await channel.send(
					new MessageEmbed()
						...
						.setTitle(`❌ **Zaten ${tickets.count} veya daha fazla açık biletiniz var**`)
						.setDescription(`Gereksiz biletleri kapatmak için bir sunucu kanalında \`${config.prefix}close\` kullanın.\n\n${ticketList.join(',\n')}`)
						.setFooter(channel.guild.name + ' | Bu mesaj 15 saniye içinde silinecek', channel.guild.iconURL())
				);
		...
		channel.guild.channels.create(name, {
			...
			reason: 'User yeni bir destek bileti kanalı talep etti (panel tepkisi)'
		}).then(async c => {
			Ticket.update({
				channel: c.id
			}, {
			...
			await c.send(ping + `${u} yeni bir bilet oluşturdu`);
			...
			if (config.logs.discord.enabled)
				client.channels.cache.get(config.logs.discord.channel).send(
					new MessageEmbed()
						.setColor(config.colour)
						.setAuthor(u.username, u.displayAvatarURL())
						.setTitle('Yeni bilet (panel üzerinden)')
						.setDescription(`\`${topic}\``)
						.addField('Yaratıcı', u, true)
						.addField('Kanal', c, true)
						.setFooter(channel.guild.name, channel.guild.iconURL())
						.setTimestamp()
				);

			log.info(`${u.tag} yeni bir bilet oluşturdu (#${name}) via panel`);
		}).catch(log.error);
	}
};
