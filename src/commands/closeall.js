...
module.exports = {
	name: 'closeall',
	description: 'Belirtilen süreden daha eski, şu anda açık olan tüm biletleri kapatır',
	usage: '[time]',
	aliases: ['ca'],
		...
		if (!message.member.roles.cache.has(config.staff_role))
			return message.channel.send(
				new MessageEmbed()
					...
					.setDescription('Personel olmadığınız için bu komutu kullanma izniniz yok.')
			...
			try {
				...
				return message.channel.send(
					new MessageEmbed()
						.setColor(config.err_colour)
						.setAuthor(message.author.username, message.author.displayAvatarURL())
						.setTitle('❌ **Geçersiz Zaman Aralığı**')
						.setDescription(`Belirttiğiniz \`${timestamp}\` zaman aralığı geçersizdi.`)
						.addField('Usage', `\`${config.prefix}${this.name}${' ' + this.usage}\`\n`)
						.addField('Help', `Type \`${config.prefix}help ${this.name}\` for more information`)
						.setFooter(guild.name, guild.iconURL())
				);
		...
		if (tickets.count === 0) 
			return message.channel.send(
				new MessageEmbed()
					...
					.setTitle('❌ **Açık ticket yok**')
					.setDescription('Kapatılacak açık ticket yok.')
					.setFooter(guild.name, guild.iconURL())
			);

		log.info(`${tickets.count} açık bilet bulundu`);
		
		if (config.commands.close.confirmation) {
			let success;
			let pre = config.transcripts.text.enabled || config.transcripts.web.enabled
				? `Her biletin arşivlenmiş bir sürümünü daha sonra \`${config.prefix}transcript <id> ile görüntüleyebileceksiniz.\``
				: '';
			...
			let confirm = await message.channel.send(
				new MessageEmbed()
					...
					.setAuthor(message.author.username, message.author.displayAvatarURL())
					.setTitle(`❔ **${tickets.count}** ticket'ı kapatmak istediğinizden emin misiniz?`)
					.setDescription(`${pre}\n**Onaylamak için ✅ ile tepkiye basınız.**`)
					.setFooter(guild.name + ' | 15 saniye içinde sona erecek', guild.iconURL())
			);
			...
			collector.on('collect', async () => {
				message.channel.send(
					new MessageEmbed()
						.setColor(config.colour)
						.setAuthor(message.author.username, message.author.displayAvatarURL())
						.setTitle(`**\`${tickets.count}\` ticket kapatıldı**`)
						.setDescription(`**\`${tickets.count}\`** ticketları kapatan ${message.author}`)
						.setFooter(guild.name, guild.iconURL())
				);
				...
				confirm.edit(
					new MessageEmbed()
						...
						.setAuthor(message.author.username, message.author.displayAvatarURL())
						.setTitle(`✅ ** \`${tickets.count}\` ticket kapatıldı**`)
						.setDescription('İçerik arşivlendikten sonra kanallar birkaç saniye içinde otomatik olarak silinecektir.')
						.setFooter(guild.name, guild.iconURL())
				);
				...
				if (config.transcripts.text.enabled || config.transcripts.web.enabled) {
					...
					if (user) {
						...
						try {
							...
							log.warn(`İle DM kanalı oluşturulamadı ${user.tag}`);
						}
						...
						if (fs.existsSync(paths.text)) {
							embed.addField('Mesaj Geçmişi', 'Özeti Gör');
							res.files = [{
						...
						try {
							...
							message.channel.send('❌ DM veya mesaj özeti gönderilemedi');
						}
					}
					...
					log.info(log.format(`${message.author.tag} ticket kapattı &7${id}&f`));

					client.channels.fetch(channel)
						.then(c => c.delete()
							.then(o => log.info(`Adlı kanal silindi: '#${o.name}' <${o.id}>`))
							.catch(e => log.error(e)))
						.catch(e => log.error(e));
					...
					if (config.logs.discord.enabled) {
						let embed = new MessageEmbed()
							.setColor(config.colour)
							.setAuthor(message.author.username, message.author.displayAvatarURL())
							.setTitle(`${tickets.count} ${plural('ticket', tickets.count)} kapatıldı (${config.prefix}closeall)`)
							.addField('Tarafından kapatıldı', message.author, true)
							.setFooter(guild.name, guild.iconURL())
							.setTimestamp();

						if (users.length > 1)
							embed.addField('Üyeler', users.map(u => `<@${u}>`).join('\n'));
